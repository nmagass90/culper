...
export default class Conferences extends SubsectionElement {
  constructor(props) {
    ...
    this.updateHasForeignConferences = this.updateHasForeignConferences.bind(
      this
    )
  ...
  summary(item, index) {
    ...
    return Summary({
      ...
      right: date,
      placeholder: i18n.t(
        'foreign.business.conferences.collection.summary.unknown'
      )
    })
  }
  ...
  render() {
    return (
      <div
        ...
        <h1 className="section-header">{i18n.t('foreign.subsection.business.events')}</h1>
        <Branch
        ...
        <Show when={this.props.HasForeignConferences.value === 'Yes'}>
          <Accordion
            ...
            description={i18n.t(
              'foreign.business.conferences.collection.summary.title'
            )}
            appendTitle={i18n.t(
              'foreign.business.conferences.collection.appendTitle'
            )}
            appendMessage={i18n.m(
              'foreign.business.conferences.collection.appendMessage'
            )}
            appendLabel={i18n.t(
              'foreign.business.conferences.collection.append'
            )}
            required={this.props.required}
            scrollIntoView={this.props.scrollIntoView}>
            <ConferencesItem
